...
bool DoosanRobot::servol(std::array<float, NUM_TASK> & posTarget, std::array<float, 2> maxSpeed, std::array<float, 2> maxAcceleration, float fTargetTime)
{
    return Drfl.servol(posTarget.data(), maxSpeed.data(), maxAcceleration.data(), fTargetTime);
}